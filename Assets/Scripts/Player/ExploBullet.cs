    ...
    {      
        for (int i = 0; i < Physics.OverlapSphere(transform.position, range).Length; i++)
        {
            ...
            if (Physics.OverlapSphere(transform.position, range)[i].GetComponent<AcR_Body>() != null)
            {
                ...
                //Adjust the face to current events
                if (Physics.OverlapSphere(transform.position, range)[i].GetComponent<MoodManager>() != null)
                    Physics.OverlapSphere(transform.position, range)[i].GetComponent<MoodManager>().FaceUpdate(2);
            }
